<template>
	<view>
		<camera :device-position="device" :flash="flash" @error="error" :style="{ width: '100%',position: 'relative', height: getHeight + 'px' }">
			...
			<cover-image @click="xzBtn" class="xzImg" src="https://mp-weixin-oss.oss-cn-beijing.aliyuncs.com/xz.png"></cover-image>
				...
				<cover-view>
					<cover-view @click="ViewImage(index)" class="imgBox" v-for="(item,index) in imgList" :key="index">
						...
						<cover-view class="cu-tag" @tap.stop="DelImg" :data-index="index">
							<cover-image class="iconClose" src="https://mp-weixin-oss.oss-cn-beijing.aliyuncs.com/icon_close.png" mode="aspectFill"></cover-image>
						</cover-view>
					</cover-view>
				</cover-view>
	...
	export default {
		data() {
			return {
				...
				nowTime2: '', //时间
				address: '',	//当前地址信息
				sgdUrl: 'https://mp-weixin-oss.oss-cn-beijing.aliyuncs.com/sgd.png',
				imgList: [
					// {
					// 	src: "https://mp-weixin-oss.oss-cn-beijing.aliyuncs.com/angular.jpg"
					// }
				],
				imgListData: '',
		...
		methods: {
			...
			sgdBtn() {
				if (this.flash == 'off') {
					this.flash = 'on'
					this.sgdUrl = 'https://mp-weixin-oss.oss-cn-beijing.aliyuncs.com/sgd_on.png'
				} else {
					this.flash = 'off'
					this.sgdUrl = 'https://mp-weixin-oss.oss-cn-beijing.aliyuncs.com/sgd.png'
				}
			},
			DelImg(e) {
				uni.showModal({
